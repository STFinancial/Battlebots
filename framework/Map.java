	...
	public void senseAllTiles() {
		...
		for (int dx = -senseRadius; dx <= senseRadius; dx++) {
			for (int dy = -senseRadius; dy <= senseRadius; dy++) {
				...
				tile = rc.senseTerrainTile(scanLoc);// If we try to scan
													// something out of range,
													// returns null
				if (tile != null) { // like behind soldiers and corners that are
									// out of range
	...
	public void senseTileAfterMove(Direction dir) {
		...
		for (int i = 0; i < toScan.length; i++) {
			...
			if (sensed[x][y])
				continue;
			scanLoc = loc.add(dx, dy);
			tile = rc.senseTerrainTile(scanLoc);// If we try to scan something
